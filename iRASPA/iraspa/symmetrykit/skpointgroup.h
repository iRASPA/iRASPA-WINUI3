...
public:
  ...
  SKPointGroup(SKRotationalOccuranceTable table, int64_t number, RKString symbol, RKString schoenflies, Holohedry holohedry, Laue laue, bool centrosymmetric, bool enantiomorphic);
  ...
  RKString holohedryString() const;
  RKString LaueString() const;
  RKString symbol() {return _symbol;}
  RKString schoenflies() {return _schoenflies;}
...
private:
  ...
  friend BinaryArchive &operator<<(BinaryArchive &, const SKPointGroup &);
  friend BinaryArchive &operator>>(BinaryArchive &, SKPointGroup &);
};
