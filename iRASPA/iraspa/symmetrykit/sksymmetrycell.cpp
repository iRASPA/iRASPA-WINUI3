...
SKSymmetryCell SKSymmetryCell::idealized(int pointGroupNumber, RKString qualifier)
{
...
std::optional<std::pair<SKSymmetryCell, SKTransformationMatrix >> SKSymmetryCell::computeReducedNiggliCellAndChangeOfBasisMatrix()
{
  ...
  // Every branch below is homogeneous of degree one in (A, B, C, xi, eta, zeta), so scaling all six by one
  // positive factor leaves the integer change-of-basis matrices it picks unchanged. Scaling by a characteristic
  // squared length (the geometric mean of A, B, C) makes the fixed epsilon a relative tolerance rather than an
  // absolute one, which is what Grosse-Kunstleve et al. (2004) recommend: a tolerance in absolute squared
  // lengths is meaningless for a large cell and too strict for a small one. The quantities are restored to
  // absolute units before the reduced cell is built.
  ...
  algorithmStart:
  {
    ...
    {
      // Whenever the product below is negative one of the three is zero and names p, but the index has to be
      // in range even on the path where it is not.
      size_t p = 0;
      std::array<int,3> f = {1,1,1};
  ...
  // Back to absolute squared lengths; the angles do not depend on the scale.
  A *= scale;
  B *= scale;
  C *= scale;
  xi *= scale;
  ...
  // Rounding can push the ratio just past the ends of the cosine's range for a degenerate cell, and an acos
  // there is a NaN angle.
  auto safeAcosDegrees = [](double cosine)
  { return acos(std::clamp(cosine, -1.0, 1.0)) * 180.0/M_PI; };

  SKSymmetryCell cell = SKSymmetryCell(sqrt(A), sqrt(B), sqrt(C),
                                       safeAcosDegrees(xi/(2.0*sqrt(B)*sqrt(C))),
                                       safeAcosDegrees(eta/(2.0*sqrt(A)*sqrt(C))),
                                       safeAcosDegrees(zeta/(2.0*sqrt(A)*sqrt(B))));
