...
{
  // Skip stale index paths that no longer resolve (e.g. after removals);
  // a nullptr in this set crashes every renderSelected* consumer.
  std::set<std::shared_ptr<SKAtomTreeNode>> selectedAtomTreeNodes{};
  for (const IndexPath &indexPath : _selectionIndexPaths)
  {
    if (std::shared_ptr<SKAtomTreeNode> node = nodeAtIndexPath(indexPath))
    {
...
BinaryArchive &operator<<(BinaryArchive & stream, const std::shared_ptr<SKAtomTreeController>& controller)
{
...
BinaryArchive &operator>>(BinaryArchive & stream, std::shared_ptr<SKAtomTreeController>& controller)
{
