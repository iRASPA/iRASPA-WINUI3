...
public:
    ...
    void setRootNodes(const std::vector<std::shared_ptr<SKAtomTreeNode>> &nodes);
...
private:
    ...
    friend BinaryArchive &operator<<(BinaryArchive &, const std::shared_ptr<SKAtomTreeController> &);
    friend BinaryArchive &operator>>(BinaryArchive &, std::shared_ptr<SKAtomTreeController> &);
};
