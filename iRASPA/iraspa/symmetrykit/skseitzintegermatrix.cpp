...
std::vector<SKSeitzIntegerMatrix> SKSeitzIntegerMatrix::SeitzMatrices(std::string encoding)
{
  ...
  //std::cerr << RKString::fromStdString(encoding) << ", " << m;
...
// A group is generated by at most three operations, and the origin-shift search
// reads all three; the ones a setting does not name are the identity.
std::vector<SKSeitzIntegerMatrix> SKSeitzIntegerMatrix::SeitzMatricesFromGenerators(std::string encoding)
{
  size_t m = std::min(encoding.size()/3, size_t(3));
