...
std::vector<SKSecondaryStructureType> SKPSeaSecondaryStructureAssigner::assign(const SKStrideBackboneChain &chain)
{
  ...
  // The Qt version returns the padded vector, which shifts every residue after the first break.
  auto perResidue = [&positions](const std::vector<SKSecondaryStructureType> &padded)
  {
    ...
    for (size_t index = 0; index < positions.size(); ++index)
    {
      ...
      {
        assignments.push_back(padded[index]);
      }
    }
    return assignments;
  };
  ...
  if (positions.size() <= 5)
  {
    return perResidue(std::vector<SKSecondaryStructureType>(positions.size(), SKSecondaryStructureType::coil));
  }
  ...
  return perResidue(mapToTypes(positions.size(), helixMask, sheetMask));
}
