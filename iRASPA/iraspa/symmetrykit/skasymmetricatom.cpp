...
SKAsymmetricAtom::SKAsymmetricAtom(RKString displayName, int elementIdentifier): _displayName(displayName), _elementIdentifier(elementIdentifier)
{
  _uniqueForceFieldName = PredefinedElements::predefinedElements[elementIdentifier]._chemicalSymbol;
}

SKAsymmetricAtom::SKAsymmetricAtom(RKString displayName, int elementIdentifier, double occupancy): _displayName(displayName), _elementIdentifier(elementIdentifier), _occupancy(occupancy)
{
  _uniqueForceFieldName = PredefinedElements::predefinedElements[elementIdentifier]._chemicalSymbol;
}
...
BinaryArchive &operator>>(BinaryArchive & stream, std::vector<std::shared_ptr<SKAtomCopy>>& val)
{
  int32_t vecSize;
  val.clear();
  stream >> vecSize;
...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<SKAsymmetricAtom> &asymmetricAtom)
{
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<SKAsymmetricAtom> &asymmetricAtom)
{
