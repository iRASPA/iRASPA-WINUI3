...
SKPointGroup::SKPointGroup(SKRotationalOccuranceTable table, int64_t number, RKString symbol, RKString schoenflies, Holohedry holohedry, Laue laue, bool centrosymmetric, bool enantiomorphic):
...
RKString SKPointGroup::holohedryString() const
{
  switch(_holohedry)
  {
  case Holohedry::none:
...
RKString SKPointGroup::LaueString() const
{
  switch(_laue)
  {
  case Laue::none:
...
std::optional<SKPointGroup> SKPointGroup::findPointGroup(double3x3 unitCell, std::vector<std::tuple<double3, int, double> > atoms, bool allowPartialOccupancies, double symmetryPrecision = 1e-2)
{
  ...
  if(primitiveDelaunayUnitCell)
  {
    ...
    // The positions have been trimmed into the primitive Delaunay cell, so that is the lattice the distance
    // test has to be made in.
...
BinaryArchive &operator<<(BinaryArchive &stream, const SKPointGroup &pointGroup)
{
  stream << pointGroup._number;
  return stream;
}

BinaryArchive &operator>>(BinaryArchive &stream, SKPointGroup &pointGroup)
{
  int64_t pointGroupNumber;
