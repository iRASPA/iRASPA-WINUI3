...
SKELFCARParser::SKELFCARParser(const std::filesystem::path &path, bool proteinOnlyAsymmetricUnitCell, bool asMolecule, CharacterSet charactersToBeSkipped): SKParser(),
...
void SKELFCARParser::startParsing() noexcept(false)
{
  ...
  // skip first line
  _scanner.scanUpToCharacters(CharacterSet::newlineCharacterSet(), scannedLine);
  if(scannedLine.empty()) {throw std::runtime_error("POSCAR file near empty");}
  ...
  for(int k=0; k<std::min(amountList.size(),elementList.size()); k++)
  {
    ...
    RKString chemicalElementString = elementList[k];
    ...
    for(int i=0; i<numberOfAtoms;i++)
    {
      ...
      RKString chemicalElement = chemicalElementString.simplified().toLower();
