...
BinaryArchive &operator<<(BinaryArchive & stream, const std::vector<std::shared_ptr<SKAtomTreeNode>>& val)
{
...
BinaryArchive &operator>>(BinaryArchive & stream, std::vector<std::shared_ptr<SKAtomTreeNode>>& val)
{
  int32_t vecSize;
  val.clear();
  stream >> vecSize;
...
// Archives written before the kind was stored name their groups by convention, which is what the
// tree was read by at the time. The names are used once more here, to recover the kinds.
SKAtomTreeGroupKind SKAtomTreeNode::groupKindFromLegacyName(const RKString &displayName)
{
  if(displayName.startsWith("Chain")) return SKAtomTreeGroupKind::chain;
  if(displayName.startsWith("Alpha-helix") ||
     displayName.startsWith("Beta-sheet") ||
     displayName.startsWith("Coil")) return SKAtomTreeGroupKind::secondaryStructureSegment;
  if(displayName.startsWith("DNA helix")) return SKAtomTreeGroupKind::dnaHelix;
  if(displayName == RKString("HETATM")) return SKAtomTreeGroupKind::hetatm;
  if(displayName == RKString("Other nucleotides")) return SKAtomTreeGroupKind::otherNucleotides;
  if(displayName == RKString("Other")) return SKAtomTreeGroupKind::other;
  return SKAtomTreeGroupKind::custom;
}

BinaryArchive &operator<<(BinaryArchive & stream, const std::shared_ptr<SKAtomTreeNode>& node)
{
  stream << node->_versionNumber;
  stream << node->_displayName;
  stream << node-> _isGroup;
  stream << node-> _isEditable;
  stream << static_cast<int64_t>(node->_groupKind);
...
BinaryArchive &operator>>(BinaryArchive & stream, std::shared_ptr<SKAtomTreeNode>& node)
{
  ...
  if(versionNumber >= 2)
  {
    int64_t groupKind;
    stream >> groupKind;
    node->_groupKind = static_cast<SKAtomTreeGroupKind>(groupKind);
  }
  else
  {
    node->_groupKind = node->_isGroup ? SKAtomTreeNode::groupKindFromLegacyName(node->_displayName)
                                      : SKAtomTreeGroupKind::none;
  }
  ...
  // A residue was named after itself rather than after what it is, so it is known only from what
  // holds it. The children are read before their parent, which is where that becomes answerable.
  if(versionNumber < 2 &&
      ...
      node->_groupKind == SKAtomTreeGroupKind::otherNucleotides))
  {
    for(const std::shared_ptr<SKAtomTreeNode> &child : node->_childNodes)
    {
      if(child->_groupKind == SKAtomTreeGroupKind::custom) child->_groupKind = SKAtomTreeGroupKind::residue;
    }
  }
