...
SKGaussianCubeParser::SKGaussianCubeParser(const std::filesystem::path &path, bool proteinOnlyAsymmetricUnitCell, bool asMolecule, CharacterSet charactersToBeSkipped): SKParser(),
  _scanner(path, charactersToBeSkipped), _proteinOnlyAsymmetricUnitCell(proteinOnlyAsymmetricUnitCell),_asMolecule(asMolecule), _frame(std::make_shared<SKStructure>()), _spaceGroupHallNumber(1)
{
...
void SKGaussianCubeParser::startParsing() noexcept(false)
{
  ...
  std::cerr << "CHECK1" << scannedLine;
  _scanner.scanLine(scannedLine);
std::cerr << "CHECK2" << scannedLine;

  // read number of atoms and origin
  _scanner.scanUpToCharacters(CharacterSet::newlineCharacterSet(), scannedLine);
  std::cerr << "CHECK" << scannedLine;
  termsScannedLined = scannedLine.splitWhitespace();
  if(termsScannedLined.empty()) {throw std::runtime_error("Empty line in cube file (missing number of atoms and origin)");}
  ...
  _frame->byteData = RKByteArray(reinterpret_cast<const uint8_t*>(dataPoints.data()), reinterpret_cast<const uint8_t*>(dataPoints.data()) + dataPoints.size() * sizeof(float));
