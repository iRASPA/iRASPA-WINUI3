...
namespace
{
  ...
  std::optional<DNABackboneAtomRole> dnaBackboneAtomRoleForName(const RKString &atomName)
  {
    const RKString name = SKNucleotide::normalizedAtomName(atomName);
    if (name == RKStringLiteral("P")) return DNABackboneAtomRole::phosphate;
    if (name == RKStringLiteral("O5'")) return DNABackboneAtomRole::o5Prime;
    if (name == RKStringLiteral("C5'")) return DNABackboneAtomRole::c5Prime;
    if (name == RKStringLiteral("C4'")) return DNABackboneAtomRole::c4Prime;
    if (name == RKStringLiteral("O4'")) return DNABackboneAtomRole::o4Prime;
    if (name == RKStringLiteral("C1'")) return DNABackboneAtomRole::c1Prime;
    if (name == RKStringLiteral("C2'")) return DNABackboneAtomRole::c2Prime;
    if (name == RKStringLiteral("C3'")) return DNABackboneAtomRole::c3Prime;
    if (name == RKStringLiteral("O3'")) return DNABackboneAtomRole::o3Prime;
    return std::nullopt;
  }
...
DNABackboneResidue::DNABackboneResidue(RKString residueName,
                                       int64_t residueSequenceNumber,
                                       char codeForInsertionOfResidues,
                                       std::shared_ptr<SKAsymmetricAtom> phosphate,
                                       std::shared_ptr<SKAsymmetricAtom> o5Prime,
...
DNABackboneChain::DNABackboneChain(char chainIdentifier, std::vector<DNABackboneResidue> residues):
...
DNABackbone DNABackbone::build(const std::vector<std::shared_ptr<SKAsymmetricAtom>> &atoms)
{
  ...
  std::map<char, std::map<ResidueKey, MutableResidue>> residuesByChain;
  std::map<char, std::vector<ResidueKey>> residueOrderByChain;
  ...
  for (const char chainId : chainOrder)
  {
