...
ProteinCrystal::ProteinCrystal(std::shared_ptr<SKStructure> frame): Structure(frame)
{
  ...
  expandSymmetry();
  // The hierarchy reorders the atoms, so it is applied before the tags that name them by position
  // and before the backbone is read off the tree.
...
ProteinCrystal::ProteinCrystal(const std::shared_ptr<Object> object): Structure(object)
{
  ...
  if(std::shared_ptr<ProteinRibbonMixin> ribbonSource = std::dynamic_pointer_cast<ProteinRibbonMixin>(object))
  {
    cloneRibbonStateFrom(*ribbonSource);
  }
  rebuildBackbone();
}
  ...
  for(std::shared_ptr<SKAsymmetricBond> asymmetricBond : asymmetricBonds)
  {
    ...
    if(isVisble)
    {
      ...
      for(std::shared_ptr<SKBond> bond : bonds)
      {
        ...
        {
          RKColor color1 = bond->atom1()->parent()->color();
          RKColor color2 = bond->atom2()->parent()->color();
...
void ProteinCrystal::setAtomSymmetryData(double3x3 unitCell, std::vector<std::tuple<double3, int, double>> atomData)
{
  ...
  for(const std::tuple<double3, int, double> &tuple : atomData)
  {
    ...
    RKString displayName = PredefinedElements::predefinedElements[elementIdentifier]._chemicalSymbol;
    std::shared_ptr<SKAsymmetricAtom> asymmetricAtom = std::make_shared<SKAsymmetricAtom>(displayName, elementIdentifier, occupancy);
    // These atoms replace the ones the symmetry search consumed, so they have to
    // be given the radius this structure draws the element at; the colour and the
    // bond-distance criterion are reapplied by the caller.
    asymmetricAtom->setDrawRadius(drawRadius(elementIdentifier));
...
std::shared_ptr<Structure> ProteinCrystal::removeSymmetry() const
{
  ...
  for(const std::shared_ptr<SKAtomTreeNode> &node: asymmetricAtomNodes)
  {
    if(node->isLeaf())
    {
      // An atom becomes the folder over its own symmetry copies, which is a group of no kind the
      // protein hierarchy knows.
      node->setGroupKind(SKAtomTreeGroupKind::custom);
...
void ProteinCrystal::computeBonds()
{
  ...
  positions.reserve(copies.size());
  for(const std::shared_ptr<SKAtomCopy> &copy : copies)
  {
    positions.push_back(copy->position());
  }

  std::vector<std::shared_ptr<SKBond>> bonds = SKBondGenerator::periodicBonds(copies, positions, _cell);
  _bondSetController->setBonds(bonds);
}
              ...
              for (int k2 = minimumReplicaY;k2 <= maximumReplicaY;k2++)
              {
                for (int k3 = minimumReplicaZ;k3 <= maximumReplicaZ;k3++)
                {
                  ...
                  case RKTextType::position:
                    text = "(" + RKString::number(copy->position().x) + "," + RKString::number(copy->position().y) + "," + RKString::number(copy->position().z) + ")";
                    break;
                  case RKTextType::charge:
                    text = RKString::number(atom->charge());
                    break;
                  }
                  ...
                  RKRectF rect = RKRectF();
                  for (int i = 0 ; i < text.size(); i++)
                  {
                    int id = fontAtlas->characterIndex[static_cast<uint16_t>(static_cast<unsigned char>(text[i]))];
                    if(id >= 0 && id<=255)
                    {
                      ...
                      rect=rect.united(RKRectF(x2,y2,w,h));
...
std::vector<float> ProteinCrystal::gridData()
{
  ...
  catch (std::exception const& e)
  {
    ...
    std::cerr << RKString::fromUtf8(e.what());
...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<ProteinCrystal> &proteinCrystal)
{
  stream << proteinCrystal->_versionNumber;

  stream << proteinCrystal->_spaceGroup;
  stream << int64_t(0x6f6b6185);

  proteinCrystal->writeRibbonState(stream, proteinCrystal->_versionNumber);
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<ProteinCrystal> &proteinCrystal)
{
  ...
  if(versionNumber >= 2) // introduced in version 2
  {
    ...
    int64_t magicNumber;
    stream >> magicNumber;
    if(magicNumber != int64_t(0x6f6b6185))
    {
  ...
  finalizeRibbonRepresentationStyleAfterLoad(*proteinCrystal, versionNumber);
  // Archives written with the hierarchy already carry it; older flat ones are migrated here.
  ProteinAtomTreeBuilder::applyHierarchyIfNeeded(*proteinCrystal->atomsTreeController(),
                                                 proteinCrystal->ribbonSecondaryStructureMethod());
