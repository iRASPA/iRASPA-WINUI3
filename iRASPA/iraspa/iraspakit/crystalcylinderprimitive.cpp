...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<CrystalCylinderPrimitive> &primitive)
{
  stream << primitive->_versionNumber;

  stream << int64_t(0x6f6b6190);
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<CrystalCylinderPrimitive> &primitive)
{
  ...
  else
  {
    int64_t magicNumber;
    stream >> magicNumber;
    if(magicNumber != int64_t(0x6f6b6190))
    {
