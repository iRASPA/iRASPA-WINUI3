...
namespace
{
  // One pass over the leaves, so resolving a whole mesh worth of tags costs one walk of the tree
  // rather than one walk per range.
  std::map<int64_t, std::shared_ptr<SKAtomTreeNode>> leafNodesByTag(SKAtomTreeController &controller)
  {
    std::map<int64_t, std::shared_ptr<SKAtomTreeNode>> nodesByTag;
    for (const std::shared_ptr<SKAtomTreeNode> &leaf : controller.flattenedLeafNodes())
    {
      if (const std::shared_ptr<SKAsymmetricAtom> atom = leaf->representedObject())
      {
        nodesByTag.emplace(atom->tag(), leaf);
      }
    }
    return nodesByTag;
  }

  bool areAllAncestorsVisible(const std::shared_ptr<SKAtomTreeNode> &node)
  {
...
ProteinRibbonResidueSegment::ProteinRibbonResidueSegment(char chainIdentifier,
...
bool ProteinRibbonSegmentSupport::isProteinHierarchyGroupNode(const std::shared_ptr<SKAtomTreeNode> &node)
{
  ...
  // A residue of the polymer is swept into the ribbon; one under HETATM, a water or a ligand, is
  // there to be listed and has only its atoms to show.
  return isResidueGroupNode(node) && isSecondaryStructureSegmentNode(node->parent());
}
...
// The ribbon of a group and nothing else. The nesting is read back out through
// isRibbonResidueVisible, which walks the group ancestors, so hiding a chain takes the ribbons of
// its segments and residues with it without any of them being written to.
...
// The atoms under a group, which are written to one by one: a group says of its atoms only what
// they say of themselves, so switching it is switching all of them.
void ProteinRibbonSegmentSupport::setGroupAtomsVisibility(const std::shared_ptr<SKAtomTreeNode> &node, bool isVisible)
{
  ...
  {
    if (const std::shared_ptr<SKAsymmetricAtom> atom = leaf ? leaf->representedObject() : nullptr)
    {
      atom->setVisibility(isVisible);
    }
  }
}

// A group outside the protein hierarchy, "Other" and the hand-made ones among them, has no ribbon to
// speak for and draws a single box, so that box has to mean the whole of what is under it.
...
{
  return node && areAllAncestorsVisible(node);
}
...
std::vector<std::shared_ptr<SKAtomTreeNode>> ProteinRibbonSegmentSupport::residueTreeNodesForAtomTags(SKAtomTreeController &controller,
                                                                                                     const std::vector<int64_t> &alphaCarbonTags)
{
  const std::map<int64_t, std::shared_ptr<SKAtomTreeNode>> nodesByTag = leafNodesByTag(controller);
  std::vector<std::shared_ptr<SKAtomTreeNode>> residueNodes;
  residueNodes.reserve(alphaCarbonTags.size());
  for (const int64_t tag : alphaCarbonTags)
  {
    const auto leaf = nodesByTag.find(tag);
    residueNodes.push_back(leaf == nodesByTag.end() ? nullptr : enclosingResidueGroupNode(leaf->second));
  }
  return residueNodes;
}

std::vector<std::shared_ptr<SKAtomTreeNode>> ProteinRibbonSegmentSupport::segmentTreeNodesForAtomTags(SKAtomTreeController &controller,
                                                                                                      const std::vector<int64_t> &alphaCarbonTags)
{
  const std::map<int64_t, std::shared_ptr<SKAtomTreeNode>> nodesByTag = leafNodesByTag(controller);
  std::vector<std::shared_ptr<SKAtomTreeNode>> segmentNodes;
  segmentNodes.reserve(alphaCarbonTags.size());
  for (const int64_t tag : alphaCarbonTags)
  {
    const auto leaf = nodesByTag.find(tag);
    segmentNodes.push_back(leaf == nodesByTag.end() ? nullptr
                                                    : enclosingSecondaryStructureSegmentNode(leaf->second));
  }
  return segmentNodes;
}
