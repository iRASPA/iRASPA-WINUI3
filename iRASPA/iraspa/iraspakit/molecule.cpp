...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<Molecule> &molecule)
{
  stream << molecule->_versionNumber;

  stream << int64_t(0x6f6b6186);
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<Molecule> &molecule)
{
  ...
  if(versionNumber >= 2) // introduced in version 2
  {
    int64_t magicNumber;
    stream >> magicNumber;
    if(magicNumber != int64_t(0x6f6b6186))
    {
