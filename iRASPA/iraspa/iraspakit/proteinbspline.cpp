...
namespace
{
  // At any parameter only degree+1 basis functions are non-zero, so a point is a weighted sum over
  // the control points of one knot span rather than over the whole chain. Evaluating every basis
  // function of every control point instead costs O(controlPoints) per sample, and the ribbon takes
  // 24 samples per residue: a protein chain of a few hundred residues then spends minutes in here.
  struct SpanBasis
  {
    int span = 0;
    // Enough for the cubic the ribbon uses; the heap vector covers a degree this cannot hold.
    double inlineWeights[8] = {};
    std::vector<double> heapWeights;

    const double *weights() const { return heapWeights.empty() ? inlineWeights : heapWeights.data(); }
    double *weights() { return heapWeights.empty() ? inlineWeights : heapWeights.data(); }
  };

  // Piegl & Tiller, "The NURBS Book", A2.1: the span holding u, in [degree, controlPointCount-1].
  int findKnotSpan(double u, int controlPointCount, int degree, const std::vector<double> &knots)
  {
    const int lastControlPoint = controlPointCount - 1;
    if (u >= knots[static_cast<size_t>(lastControlPoint) + 1]) return lastControlPoint;
  ...
  // Piegl & Tiller A2.2: the degree+1 non-vanishing basis functions at u, without divisions by zero
  // and without the repeated work of the plain Cox-de Boor recursion.
  SpanBasis nonVanishingBasisFunctions(double u, int controlPointCount, int degree,
                                       const std::vector<double> &knots)
  {
    SpanBasis basis;
    basis.span = findKnotSpan(u, controlPointCount, degree, knots);

    const size_t count = static_cast<size_t>(degree) + 1;
    if (count > sizeof(basis.inlineWeights) / sizeof(basis.inlineWeights[0]))
    {
      basis.heapWeights.assign(count, 0.0);
    }
    double *weights = basis.weights();

    std::vector<double> leftHeap;
    std::vector<double> rightHeap;
    double leftInline[8] = {};
    double rightInline[8] = {};
    double *left = leftInline;
    double *right = rightInline;
    if (count > sizeof(leftInline) / sizeof(leftInline[0]))
    {
      leftHeap.assign(count, 0.0);
      rightHeap.assign(count, 0.0);
      left = leftHeap.data();
      right = rightHeap.data();
    }

    weights[0] = 1.0;
    for (int j = 1; j <= degree; ++j)
    {
      left[j] = u - knots[static_cast<size_t>(basis.span + 1 - j)];
      right[j] = knots[static_cast<size_t>(basis.span + j)] - u;
      double saved = 0.0;
      for (int r = 0; r < j; ++r)
      {
        const double denominator = right[r + 1] + left[j - r];
        const double term = denominator != 0.0 ? weights[r] / denominator : 0.0;
        weights[r] = saved + right[r + 1] * term;
        saved = left[j - r] * term;
      }
      weights[j] = saved;
    }
  ...
  double3 weightedSpanSum(const SpanBasis &basis, int degree, const std::vector<double3> &values)
  {
    const double *weights = basis.weights();
    double3 sum = double3(0.0, 0.0, 0.0);
    for (int j = 0; j <= degree; ++j)
    {
      sum += weights[j] * values[static_cast<size_t>(basis.span - degree + j)];
    }
    return sum;
  }
}
...
double3 ProteinBSpline::evaluateOrientation(double t) const
{
  ...
  const SpanBasis basis =
    nonVanishingBasisFunctions(t, static_cast<int>(_controlPoints.size()), _degree, _knots);
  double3 orientation = weightedSpanSum(basis, _degree, _orientationVectors);
                                      ...
                                      const std::vector<double> &knots)
{
  ...
  const SpanBasis basis =
    nonVanishingBasisFunctions(t, static_cast<int>(controlPoints.size()), degree, knots);
  return weightedSpanSum(basis, degree, controlPoints);
}
