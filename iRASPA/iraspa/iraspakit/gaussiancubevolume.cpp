...
GaussianCubeVolume::GaussianCubeVolume(std::shared_ptr<SKStructure> frame): Structure(frame)
{
    ...
    std::cerr << "_adsorptionSurfaceIsoValue" << _adsorptionSurfaceIsoValue;
...
void GaussianCubeVolume::computeBonds()
{
  ...
  // The atoms are held fractional; a bond is measured in Cartesian space.
...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<GaussianCubeVolume> &volume)
{
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<GaussianCubeVolume> &volume)
{
  ...
  int64_t magicNumber;
  stream >> magicNumber;
  if(magicNumber != int64_t(0x6f6b6199))
  {
