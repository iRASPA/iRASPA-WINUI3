...
namespace
{
  ...
  struct MutableResidue
  {
    ...
    std::map<RKString, std::shared_ptr<SKAsymmetricAtom>> atomsByName;
  };

  std::shared_ptr<SKAsymmetricAtom> atomNamed(const MutableResidue &residue, const RKString &name)
  {
...
DNANucleotideGeometry DNANucleotideGeometry::build(const std::vector<std::shared_ptr<SKAsymmetricAtom>> &atoms)
{
  ...
  for (const auto &entry : residuesByKey)
  {
    ...
    residue.c1Prime = atomNamed(mutableResidue, RKStringLiteral("C1'"));
    residue.phosphate = atomNamed(mutableResidue, RKStringLiteral("P"));
    const RKString anchorName = SKNucleotideBase::baseAnchorAtomName(residue.baseKind);
