...
std::vector<float> MolecularCrystal::gridData()
{
  ...
  try
  {
    ...
    const auto [min, max] = std::minmax_element(begin(gridData), end(gridData));
            std::cerr << "FOUND min/max: " << *min << *max;
...
std::vector<float> MolecularCrystal::wellFieldData()
{
  int size = pow(2,encompassingPowerOfTwoCubicGridSize());
  _dimensions = int3(size,size,size);
...
void MolecularCrystal::refineWellSurface(std::vector<float4> &triangleData, float trimIsovalue)
{
...
double MolecularCrystal::computeWellSurfaceAreaAccelerated() const noexcept(false)
{
  int3 size = int3(128,128,128);
...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<MolecularCrystal> &molecularCrystal)
{
  stream << molecularCrystal->_versionNumber;

  stream << molecularCrystal->_spaceGroup;
  stream << int64_t(0x6f6b6184);
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<MolecularCrystal> &molecularCrystal)
{
  ...
  if(versionNumber >= 2) // introduced in version 2
  {
    ...
    int64_t magicNumber;
    stream >> magicNumber;
    if(magicNumber != int64_t(0x6f6b6184))
    {
