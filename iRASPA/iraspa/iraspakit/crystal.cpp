...
void Crystal::computeBonds()
{
  ...
  // The atoms of a crystal are held fractional; a bond is measured in Cartesian space.
  std::vector<double3> positions;
  positions.reserve(copies.size());
  for(const std::shared_ptr<SKAtomCopy> &copy : copies)
  {
    positions.push_back(_cell->unitCell() * copy->position());
  }
...
std::vector<float> Crystal::wellFieldData()
{
  int size = pow(2,_encompassingPowerOfTwoCubicGridSize);
  _dimensions = int3(size,size,size);
...
void Crystal::refineWellSurface(std::vector<float4> &triangleData, float trimIsovalue)
{
...
double Crystal::computeWellSurfaceAreaAccelerated() const noexcept(false)
{
  int3 size = int3(128,128,128);
...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<Crystal> &crystal)
{
  stream << crystal->_versionNumber;

  stream << crystal->_spaceGroup;

  stream << int64_t(0x6f6b6183);
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<Crystal> &crystal)
{
  ...
  if(versionNumber >= 2) // introduced in version 2
  {
    ...
    int64_t magicNumber;
    stream >> magicNumber;
    if(magicNumber != int64_t(0x6f6b6183))
    {
