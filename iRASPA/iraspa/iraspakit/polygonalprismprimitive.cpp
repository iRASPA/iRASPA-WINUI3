...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<PolygonalPrismPrimitive> &primitive)
{
  stream << primitive->_versionNumber;
  stream << int64_t(0x6f6b6194);
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<PolygonalPrismPrimitive> &primitive)
{
  ...
  else
  {
    int64_t magicNumber;
    stream >> magicNumber;
    if(magicNumber != int64_t(0x6f6b6194))
    {
