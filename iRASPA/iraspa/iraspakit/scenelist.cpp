...
SceneList::SceneList(RKString displayName, std::vector<std::shared_ptr<iRASPAObject>> objects)
{
...
BinaryArchive &operator<<(BinaryArchive & stream, const std::vector<std::shared_ptr<Scene>>& val)
{
...
BinaryArchive &operator>>(BinaryArchive & stream, std::vector<std::shared_ptr<Scene>>& val)
{
  int32_t vecSize;
  val.clear();
  stream >> vecSize;
...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<SceneList> &sceneList)
{
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<SceneList> &sceneList)
{
