...
public:
    ...
    ProjectTreeNode(RKString displayName = "Default", bool isEditable=true, bool isDropEnabled=true): _type(Type::user), _displayName(displayName), _representedObject(std::make_shared<iRASPAProject>()), _isEditable(isEditable), _isDropEnabled(isDropEnabled) {}
    ...
    ProjectTreeNode(RKString displayName, std::shared_ptr<iRASPAProject> representedObject, bool isEditable=true, bool isDropEnabled=true): _type(Type::user), _displayName(displayName), _representedObject(representedObject), _isEditable(isEditable), _isDropEnabled(isDropEnabled) {}
    ~ProjectTreeNode();
    ...
    bool setData(const RKString &value);
...
private:
    ...
    friend BinaryArchive &operator<<(BinaryArchive & stream, const std::vector<std::shared_ptr<ProjectTreeNode>>& val);
    friend BinaryArchive &operator>>(BinaryArchive & stream, std::vector<std::shared_ptr<ProjectTreeNode>>& val);

    friend BinaryArchive &operator<<(BinaryArchive &, const std::shared_ptr<ProjectTreeNode> &);
    friend BinaryArchive &operator>>(BinaryArchive &, std::shared_ptr<ProjectTreeNode> &);
    friend BinaryArchive &operator<<=(BinaryArchive &, const std::shared_ptr<ProjectTreeNode> &);
    friend BinaryArchive &operator>>=(BinaryArchive &, std::shared_ptr<ProjectTreeNode> &);
};
