...
public:
  ...
  static std::shared_ptr<Movie> create(RKString displayName, std::vector<std::shared_ptr<iRASPAObject>> iraspaStructures);
  virtual ~Movie() {}
  ...
  RKString displayName() const override final;
...
private:
  ...
  RKString _displayName = RKString("Movie");

  std::weak_ptr<Scene> _parent{};
  std::vector<std::shared_ptr<iRASPAObject>> _frames{};
  FrameSelectionIndexSet _selectedFramesIndexSet;

  friend BinaryArchive &operator<<(BinaryArchive & stream, const std::vector<std::shared_ptr<iRASPAObject>>& val);
  friend BinaryArchive &operator>>(BinaryArchive & stream, std::vector<std::shared_ptr<iRASPAObject>>& val);

  friend BinaryArchive &operator<<(BinaryArchive &, const std::shared_ptr<Movie> &);
  friend BinaryArchive &operator>>(BinaryArchive &, std::shared_ptr<Movie> &);
