...
{
  std::optional<RKString> displayName = frame->displayName;
  if(displayName)
  {
  ...
  const auto *bytes = reinterpret_cast<const uint8_t *>(dataGrid.data());
  _data.assign(bytes, bytes + dataGrid.size() * sizeof(float));
}

BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<RASPADensityVolume> &volume)
{
  stream << volume->_versionNumber;

  stream << int64_t(0x6f6b6196);
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<RASPADensityVolume> &volume)
{
  ...
  int64_t magicNumber;
  stream >> magicNumber;
  if(magicNumber != int64_t(0x6f6b6196))
  {
