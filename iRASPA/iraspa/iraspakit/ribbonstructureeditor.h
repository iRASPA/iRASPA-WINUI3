...
public:
  ...
  virtual void setRibbonAmbientColor(const RKColor &value) = 0;
  virtual RKColor ribbonDiffuseColor() const = 0;
  virtual void setRibbonDiffuseColor(const RKColor &value) = 0;
  virtual RKColor ribbonSpecularColor() const = 0;
  virtual void setRibbonSpecularColor(const RKColor &value) = 0;
...
/// After archive load: honor a persisted Default/Fancy label (re-apply its lighting) instead of
/// letting recheck overwrite the label from possibly-stale lighting. Custom keeps archived lighting.
