...
{
  return PredefinedElements::backboneAtomRoleForType(type);
}

std::optional<SKBackboneAtomRole> backboneAtomRole(const std::shared_ptr<SKAsymmetricAtom> &atom)
{
  if (!atom) return std::nullopt;
  return PredefinedElements::backboneAtomRole(atom->residueName(), atom->displayName());
}

bool isBackboneAtomType(const RKString &type)
{
  return PredefinedElements::isBackboneAtomType(type);
}

ProteinBackboneResidue::ProteinBackboneResidue(RKString residueName,
...
ProteinBackboneChain::ProteinBackboneChain(char chainIdentifier, std::vector<ProteinBackboneResidue> residues):
...
ProteinBackbone ProteinBackbone::build(const std::vector<std::shared_ptr<SKAsymmetricAtom>> &atoms)
{
  ...
  for (const std::shared_ptr<SKAsymmetricAtom> &atom : atoms)
  {
    ...
    {
      const RKString atomName = atom->displayName().trimmed().toUpper();
      if (residue.carbonylOxygen)
      {
        const RKString existingName = residue.carbonylOxygen->displayName().trimmed().toUpper();
