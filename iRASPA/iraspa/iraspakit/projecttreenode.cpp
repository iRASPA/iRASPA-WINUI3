...
bool ProjectTreeNode::insertChildren(size_t position, size_t count, [[maybe_unused]] size_t columns)
{
  ...
  {
    std::shared_ptr<ProjectTreeNode> item = std::make_shared<ProjectTreeNode>(RKString("tree node"));
...
bool ProjectTreeNode::setData(const RKString &value)
{
  _displayName = value;
  return true;
}
...
BinaryArchive &operator<<(BinaryArchive & stream, const std::vector<std::shared_ptr<ProjectTreeNode>>& val)
{
...
BinaryArchive &operator>>(BinaryArchive & stream, std::vector<std::shared_ptr<ProjectTreeNode>>& val)
{
  int32_t vecSize;
  val.clear();
  stream >> vecSize;
...
BinaryArchive &operator<<(BinaryArchive & stream, const std::shared_ptr<ProjectTreeNode>& node)
{
  ...
  // legacy thumbnail payload (raw pixels; PNG decode skipped without Qt)
  stream << node->_thumbnail.pixels();
...
BinaryArchive &operator>>(BinaryArchive & stream, std::shared_ptr<ProjectTreeNode>& node)
{
  ...
  if(versionNumber >= 2) // introduced in version 2
  {
     ...
     node->_thumbnail = RKImage(64, 64, RKImage::Format_RGBA8888);
  }
