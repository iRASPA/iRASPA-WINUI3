...
std::shared_ptr<Movie> Movie::create(RKString displayName, std::vector<std::shared_ptr<iRASPAObject>> iraspaStructures)
{
...
RKString Movie::displayName() const
{
 return _displayName;
}
...
BinaryArchive &operator<<(BinaryArchive & stream, const std::vector<std::shared_ptr<iRASPAObject>>& val)
{
...
BinaryArchive &operator>>(BinaryArchive & stream, std::vector<std::shared_ptr<iRASPAObject>>& val)
{
  int32_t vecSize;
  val.clear();
  stream >> vecSize;
...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<Movie> &movie)
{
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<Movie> &movie)
{
