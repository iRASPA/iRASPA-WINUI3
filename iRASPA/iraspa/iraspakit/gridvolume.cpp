...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<GridVolume> &volume)
{
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<GridVolume> &volume)
{
     std::cerr << "BEGIN check";
  int64_t versionNumber;
  stream >> versionNumber;
  if(versionNumber > volume->_versionNumber)
  {
    std::cerr << "version" << versionNumber << volume->_versionNumber;
  ...
  int64_t magicNumber;
  stream >> magicNumber;
  if(magicNumber != int64_t(0x6f6b6195))
  {
      std::cerr << "HERE check";
