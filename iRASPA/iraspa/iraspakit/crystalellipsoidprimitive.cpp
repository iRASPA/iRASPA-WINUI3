...
{
  std::cerr << "CrystalCylinderPrimitive::superCell";
...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<CrystalEllipsoidPrimitive> &primitive)
{
  stream << primitive->_versionNumber;

  stream << int64_t(0x6f6b6189);
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<CrystalEllipsoidPrimitive> &primitive)
{
  ...
  else
  {
    int64_t magicNumber;
    stream >> magicNumber;
    if(magicNumber != int64_t(0x6f6b6189))
    {
