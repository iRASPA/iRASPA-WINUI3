...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<Object> &object)
{
  ...
  stream << int64_t(0x6f6b6181);

  return stream;
}

BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<Object> &object)
{
  uint16_t day,month;
  uint32_t year;
  ...
  stream >> month;
  stream >> year;
  object->_creationDate = RKDate(int(year), int(month), int(day));

  int64_t magicNumber;
  stream >> magicNumber;
  if(magicNumber != int64_t(0x6f6b6181))
  {
