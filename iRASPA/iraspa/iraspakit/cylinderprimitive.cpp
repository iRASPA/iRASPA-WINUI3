...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<CylinderPrimitive> &primitive)
{
  stream << primitive->_versionNumber;
  stream << int64_t(0x6f6b6193);
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<CylinderPrimitive> &primitive)
{
  ...
  else
  {
    int64_t magicNumber;
    stream >> magicNumber;
    if(magicNumber != int64_t(0x6f6b6193))
    {
