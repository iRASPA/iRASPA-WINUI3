...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<VTKDensityVolume> &volume)
{
  stream << volume->_versionNumber;

  stream << int64_t(0x6f6b6197);
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<VTKDensityVolume> &volume)
{
  ...
  int64_t magicNumber;
  stream >> magicNumber;
  if(magicNumber != int64_t(0x6f6b6197))
  {
