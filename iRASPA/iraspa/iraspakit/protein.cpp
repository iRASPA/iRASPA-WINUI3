...
void Protein::computeBonds()
{
  ...
  std::vector<std::shared_ptr<SKBond>> bonds = SKBondGenerator::bonds(copies, positions);
...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<Protein> &protein)
{
  stream << protein->_versionNumber;
  stream << int64_t(0x6f6b6187);

  protein->writeRibbonState(stream, protein->_versionNumber);
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<Protein> &protein)
{
  ...
  if(versionNumber >= 2) // introduced in version 2
  {
    int64_t magicNumber;
    stream >> magicNumber;
    if(magicNumber != int64_t(0x6f6b6187))
    {
  ...
  finalizeRibbonRepresentationStyleAfterLoad(*protein, versionNumber);
  // Archives written with the hierarchy already carry it; older flat ones are migrated here.
  ProteinAtomTreeBuilder::applyHierarchyIfNeeded(*protein->atomsTreeController(),
                                                 protein->ribbonSecondaryStructureMethod());
