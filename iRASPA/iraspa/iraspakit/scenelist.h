...
public:
  SceneList();
  SceneList(RKString displayName, std::vector<std::shared_ptr<iRASPAObject>> objects);
...
private:
  ...
  RKString _displayName = RKString("");
  ...
  friend BinaryArchive &operator<<(BinaryArchive & stream, const std::vector<std::shared_ptr<Scene>>& val);
  friend BinaryArchive &operator>>(BinaryArchive & stream, std::vector<std::shared_ptr<Scene>>& val);

  friend BinaryArchive &operator<<(BinaryArchive &, const std::shared_ptr<SceneList> &);
  friend BinaryArchive &operator>>(BinaryArchive &, std::shared_ptr<SceneList> &);
};
