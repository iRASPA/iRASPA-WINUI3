...
namespace
{
  ...
  CrossSectionProfile crossSectionProfile(const RibbonStation &station, const ProteinRibbonMeshParameters &parameters)
  {
    ...
    case ProteinRibbonSecondaryStructure::sheet:
      ...
      // (wide along widthDirection). Arrow scales only that in-plane width; thickness stays
      // constant. Oval tips collapse to a normal-axis spike and read as a thin line.
                            ...
                            int cycles)
  {
    ...
    while (index < count)
    {
      ...
      while (index < count &&
             secondaryStructure[static_cast<size_t>(index)] == ProteinRibbonSecondaryStructure::sheet)
      {
        ++index;
      }
      ...
      for (int cycle = 0; cycle < cycles; ++cycle)
      {
        for (int b = runStart + window; b <= runEnd - window; ++b)
        {
          ...
          {
            sum = sum + centers[static_cast<size_t>(b + e)];
          }
        ...
        for (int b = runStart + window; b <= runEnd - window; ++b)
        {
          ...
          for (int e = -window; e <= window; ++e)
          {
            sum = sum + orientations[static_cast<size_t>(b + e)];
          }
        ...
        {
          const double3 tangent =
            safeNormalize(centers[static_cast<size_t>(b + 1)] - centers[static_cast<size_t>(b - 1)],
  ...
  // Two consecutive alpha carbons further apart than this are not joined by a peptide bond: the
  // residues between them are missing from the file. The assigners draw the line at the same 4.5 A.
  constexpr double kChainBreakAlphaCarbonDistance = 4.5;

  // The residues that are traced, in file order. Everything downstream of the assigner is indexed
  // against these rather than against every residue, because a residue with no alpha carbon has no
  // point on the centre line.
  ProteinBackboneChain tracedResidues(const ProteinBackboneChain &chain)
  {
    ProteinBackboneChain traced;
    traced.chainIdentifier = chain.chainIdentifier;
    for (const ProteinBackboneResidue &residue : chain.residues)
    {
      if (residue.alphaCarbon) traced.residues.push_back(residue);
    }
    return traced;
  }

  // Half-open residue ranges that are each continuous, split wherever the chain breaks. A ribbon swept
  // across a break would run as one straight band through the missing residues, so each range is
  // swept on its own. iRASPA-QT sweeps the whole chain and shows that band.
  std::vector<std::pair<size_t, size_t>> continuousResidueRanges(const ProteinBackboneChain &traced)
  {
    ...
    {
      const double3 previous = traced.residues[index - 1].alphaCarbon->position();
      const double3 current = traced.residues[index].alphaCarbon->position();
      if (length(current - previous) > kChainBreakAlphaCarbonDistance)
      {
        ranges.emplace_back(start, index);
        start = index;
      }
    }
    ranges.emplace_back(start, traced.residues.size());
    return ranges;
  }
  ...
  void applyRibbonStripeCoordinates(RKRibbonMesh &mesh)
  {
    for (const RKRibbonChainDrawRange &drawRange : mesh.residueDrawRanges)
    {
      ...
      {
        if (indexOffset < 0 || indexOffset >= static_cast<int>(mesh.indices.size())) continue;
        const uint32_t vertexIndex = mesh.indices[static_cast<size_t>(indexOffset)];
        if (vertexIndex >= mesh.vertices.size()) continue;
        const float alongChain = mesh.vertices[vertexIndex].st.x;
  ...
  void applyPickingIndices(RKRibbonMesh &mesh)
  {
    for (size_t segmentIndex = 0; segmentIndex < mesh.segmentDrawRanges.size(); ++segmentIndex)
    {
      ...
      for (int indexOffset = drawRange.indexStart; indexOffset < end; ++indexOffset)
      {
        ...
        const uint32_t vertexIndex = mesh.indices[static_cast<size_t>(indexOffset)];
        if (vertexIndex >= mesh.vertices.size()) continue;
        mesh.vertices[vertexIndex].normal.w = static_cast<float>(segmentIndex);
      }
    }
    for (size_t residueIndex = 0; residueIndex < mesh.residueDrawRanges.size(); ++residueIndex)
    {
      ...
      for (int indexOffset = drawRange.indexStart; indexOffset < end; ++indexOffset)
      {
        ...
        const uint32_t vertexIndex = mesh.indices[static_cast<size_t>(indexOffset)];
        if (vertexIndex >= mesh.vertices.size()) continue;
        mesh.vertices[vertexIndex].pad.y = static_cast<float>(residueIndex);
      }
    }
  }

  // The alpha carbons a swept chain is stationed on, in the order the stations count them.
  std::vector<std::shared_ptr<SKAsymmetricAtom>> alphaCarbonsOfChain(const ProteinBackboneChain &chain)
  {
  ...
  int64_t alphaCarbonTag(const std::vector<std::shared_ptr<SKAsymmetricAtom>> &alphaCarbons, int index)
  {
    if (index < 0 || index >= static_cast<int>(alphaCarbons.size())) return -1;
    return alphaCarbons[static_cast<size_t>(index)]->tag();
  }

  // The first and last station of every residue, in one pass over the stations. Both the residue
  // and the segment ranges below need this, and rescanning every station once per residue (24 of
  // them per residue, a few hundred residues to a chain) is what made a large protein crawl.
  struct ResidueStationSpans
  {
    std::vector<int> firstStation;
    std::vector<int> lastStation;
  };

  ResidueStationSpans residueStationSpans(const std::vector<RibbonStation> &stations, int residueCount)
  {
    ResidueStationSpans spans;
    spans.firstStation.assign(static_cast<size_t>(residueCount), -1);
    spans.lastStation.assign(static_cast<size_t>(residueCount), -1);
    for (size_t stationIndex = 0; stationIndex < stations.size(); ++stationIndex)
    {
      ...
      if (residueIndex < 0 || residueIndex >= residueCount) continue;
      if (spans.firstStation[static_cast<size_t>(residueIndex)] < 0)
      {
        spans.firstStation[static_cast<size_t>(residueIndex)] = static_cast<int>(stationIndex);
      }
      spans.lastStation[static_cast<size_t>(residueIndex)] = static_cast<int>(stationIndex);
    }
    return spans;
  }
                               ...
                               RKRibbonMesh &mesh)
  {
    ...
    const std::vector<std::shared_ptr<SKAsymmetricAtom>> alphaCarbons = alphaCarbonsOfChain(chain);
    const int residueCount = static_cast<int>(alphaCarbons.size());
    if (residueCount <= 0) return;

    const ResidueStationSpans spans = residueStationSpans(stations, residueCount);
    const int indicesPerRingPair = ringResolution * 6;
    for (int residueIndex = 0; residueIndex < residueCount; ++residueIndex)
    {
      const int ringStart = spans.firstStation[static_cast<size_t>(residueIndex)];
      const int ringEnd = spans.lastStation[static_cast<size_t>(residueIndex)];
      if (ringStart < 0 || ringEnd <= ringStart) continue;
      int drawEndRing = ringEnd;
      if (residueIndex + 1 < residueCount)
      {
        drawEndRing = std::min(static_cast<int>(stations.size()) - 1, ringEnd + 1);
      }
      mesh.residueDrawRanges.emplace_back(chainIndexStart + ringStart * indicesPerRingPair,
                                          (drawEndRing - ringStart) * indicesPerRingPair);
      mesh.residueAlphaCarbonTags.push_back(alphaCarbonTag(alphaCarbons, residueIndex));
    }
  }
                               ...
                               RKRibbonMesh &mesh)
  {
    ...
    for (size_t segmentIndex = 0; segmentIndex < residueSegments.size(); ++segmentIndex)
    {
      ...
      const int firstResidue = std::max(segment.firstResidueIndex, 0);
      const int lastResidue = std::min(segment.lastResidueIndex, residueCount - 1);
      for (int residueIndex = firstResidue; residueIndex <= lastResidue; ++residueIndex)
      {
        const int residueFirst = spans.firstStation[static_cast<size_t>(residueIndex)];
        const int residueLast = spans.lastStation[static_cast<size_t>(residueIndex)];
        if (residueFirst < 0) continue;
        ringStart = ringStart.has_value() ? std::min(*ringStart, residueFirst) : residueFirst;
        ringEnd = ringEnd.has_value() ? std::max(*ringEnd, residueLast) : residueLast;
      }
      ...
      mesh.segmentDrawRanges.emplace_back(chainIndexStart + *ringStart * indicesPerRingPair,
                                          (drawEndRing - *ringStart) * indicesPerRingPair);
      mesh.segmentAlphaCarbonTags.push_back(alphaCarbonTag(alphaCarbons, segment.firstResidueIndex));
    }
  }

  // A chain broken into fragments is swept one fragment at a time, but its lightmap strip belongs to
  // the whole chain, so a fragment is told where it starts within the chain and how many rings the
  // chain has in total. Left at the defaults the ring coordinate is the fragment's own, which is what
  // an unbroken chain wants.
  struct RingMeshBuild
  {
    std::vector<RKRibbonVertex> vertices;
    std::vector<uint32_t> indices;
  };

  void appendRemappedIndices(RKRibbonMesh &mesh, const std::vector<uint32_t> &localIndices, uint32_t vertexBase)
  {
    if (vertexBase == 0)
    {
  ...
  RingMeshBuild ringMesh(const std::vector<RibbonStation> &stations,
                         double radius,
                         int chainIndex,
                         int totalChains,
                         int ringResolution,
                         ...
                         int chainRingTotal = 0)
  {
    ...
    for (const RibbonStation &station : stations)
    {
      ...
      constexpr double seamAngleOffset = 0.5 * M_PI;

      for (int segment = 0; segment < loopResolution; ++segment)
      {
        const double angle =
          2.0 * M_PI * static_cast<double>(segment) / static_cast<double>(loopResolution) + seamAngleOffset;
        double3 offset;
        if (parameters.nucleicAcidRendering)
        {
        ...
        else
        {
          ...
          if (profile.rectangular)
          {
            ...
            // widthClamp is in-plane half-width (body 1, arrow peak > 1, tip → coil).
          ...
          else
          {
            ...
            {
              width = std::max(-profile.widthClamp, std::min(profile.widthClamp, width));
            }
    ...
    std::vector<RKRibbonVertex> meshVertices(totalVertices);
    ...
    for (int ringIndex = 0; ringIndex < splineSamples; ++ringIndex)
    {
      ...
      for (int segment = 0; segment <= loopResolution; ++segment)
      {
        ...
        meshVertices[vertexIndex].position = float4(static_cast<float>(position.x),
                                                    ...
                                                    1.0f);
        meshVertices[vertexIndex].st = parametricLightmapUV(ringOffsetWithinChain + ringIndex, lightmapRingTotal,
                                                            arcLengthFraction, chainIndex, totalChains);
    ...
    std::vector<uint32_t> indices(static_cast<size_t>((splineSamples - 1) * loopResolution * 6));
    ...
    return {std::move(meshVertices), std::move(indices)};
  }
}
                                             ...
                                             ProteinRibbonSecondaryStructureMethod secondaryStructureMethod)
{
  ...
  // A fragment is one continuous run of residues. An unbroken chain is a single fragment, so it is
  // swept exactly as before; a broken one is swept once per run and the ribbon stops at the break.
  ...
  // Rings per drawn chain, for the lightmap strip a chain's fragments share.
  ...
  for (const ProteinBackboneChain &chain : backbone.chains)
  {
    ...
    // The assignment is made over the whole chain, breaks and all. The assigners handle a break
    // themselves, and assigning per fragment would hide from them that the residues are related.
    const std::vector<ProteinRibbonSecondaryStructure> secondaryStructure =
    ...
    int chainRings = 0;
    for (const std::pair<size_t, size_t> &range : continuousResidueRanges(traced))
    {
      const size_t first = range.first;
      const size_t last = std::min(range.second, secondaryStructure.size());
      if (last <= first) continue;

      ProteinBackboneChain fragmentChain;
      fragmentChain.chainIdentifier = traced.chainIdentifier;
      fragmentChain.residues.assign(traced.residues.begin() + static_cast<std::ptrdiff_t>(first),
                                    traced.residues.begin() + static_cast<std::ptrdiff_t>(last));

      const std::vector<ProteinRibbonSecondaryStructure> fragmentStructure(
        secondaryStructure.begin() + static_cast<std::ptrdiff_t>(first),
        secondaryStructure.begin() + static_cast<std::ptrdiff_t>(last));
      ...
      chainFragments.push_back({fragmentChain, stations, residueSegments, chainRingTotals.size()});
    }

    if (chainFragments.empty()) continue;
    chainRingTotals.push_back(chainRings);
    fragments.insert(fragments.end(), chainFragments.begin(), chainFragments.end());
  }
  ...
  for (const FragmentStationData &fragment : fragments)
  {
    if (!haveSlot || fragment.chainSlot != currentSlot)
    {
      if (haveSlot)
      {
        ...
        mesh.chainSplineSampleCounts.push_back(chainRingTotals[currentSlot]);
      }
    ...
    const RingMeshBuild built = ringMesh(fragment.stations,
                                         radius,
                                         static_cast<int>(fragment.chainSlot),
                                         totalChains,
                                         ringResolution,
                                         meshParameters,
                                         ringOffset,
                                         chainRingTotals[fragment.chainSlot]);
    mesh.vertices.insert(mesh.vertices.end(), built.vertices.begin(), built.vertices.end());
    appendRemappedIndices(mesh, built.indices, vertexBase);
                                                              ...
                                                              ProteinRibbonMeshParameters parameters)
{
  ...
  for (size_t chainIndex = 0; chainIndex < chainStations.size(); ++chainIndex)
  {
    ...
    const int indexStart = static_cast<int>(mesh.indices.size());
    const uint32_t vertexBase = static_cast<uint32_t>(mesh.vertices.size());
    const RingMeshBuild built = ringMesh(chainData.stations,
                                         radius,
