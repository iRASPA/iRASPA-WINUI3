...
uint16_t RKHalfFloat::halfBitsFromFloat(float value)
{
  ...
  return static_cast<uint16_t>(half | static_cast<uint16_t>(halfExponent << 10) |
...
void RibbonAOTexturePostProcess::gaussianBlur(std::vector<float> &data, int width, int height)
{
  ...
  // A chain occupies one 32-texel strip but stretches over the whole atlas width, so the two axes
  // are nowhere near the same scale in ribbon space: taps eight texels apart along the spline cover
  // about as much ribbon as taps one texel apart across it.
  ...
  for (int y = 0; y < height; ++y)
  {
    for (int x = 0; x < width; ++x)
    {
      ...
      for (int i = 0; i < 7; ++i)
      {
        ...
        sum += sampleClamped(data, width, height, static_cast<float>(x) - offset, static_cast<float>(y)) *
               weights[i + 1];
        sum += sampleClamped(data, width, height, static_cast<float>(x) + offset, static_cast<float>(y)) *
               weights[i + 1];
      }
  ...
  for (int y = 0; y < height; ++y)
  {
    for (int x = 0; x < width; ++x)
    {
      ...
      for (int i = 0; i < 7; ++i)
      {
        ...
        sum += sampleClamped(data, width, height, static_cast<float>(x), static_cast<float>(y) - offset) *
               weights[i + 1];
        sum += sampleClamped(data, width, height, static_cast<float>(x), static_cast<float>(y) + offset) *
               weights[i + 1];
      }
...
void RibbonAOTexturePostProcess::dilateAndSmooth(std::vector<float> &data, int width, int height,
                                                 int smoothPasses)
{
  if (width <= 0 || height <= 0 || static_cast<int>(data.size()) < width * height) { return; }
...
void RibbonAOTexturePostProcess::smooth(const std::vector<float> &source, std::vector<float> &data,
...
void RibbonAOTexturePostProcess::smoothAlongStrand(const std::vector<float> &source,
