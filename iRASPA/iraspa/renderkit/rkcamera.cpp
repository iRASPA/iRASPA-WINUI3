...
double3 RKCamera::myGluUnProject(double3 position, RKRect viewPort)
{
...
double3 RKCamera::myGluUnProject(double3 position, double4x4 mMatrix, RKRect viewPort)
{
  double4x4 finalMatrix;
  double4 inVector = double4();
...
double3 RKCamera::myGluProject(double3 position, RKRect viewPort)
{
...
std::vector<size_t> RKCamera::selectPositionsInRectangle(std::vector<double3> &positions, RKRect rect, double3 origin, RKRect viewPortBounds)
{
  // (debug logging removed — QRect has no std::ostream operator)
...
BinaryArchive &operator<<(BinaryArchive &stream, const std::shared_ptr<RKCamera> &camera)
{
...
BinaryArchive &operator>>(BinaryArchive &stream, std::shared_ptr<RKCamera> &camera)
{
  ...
  stream >> camera->_orthoScale;
  stream >> camera->_angleOfView;
  int64_t frustrumType;
  stream >> frustrumType;
  camera->_frustrumType = FrustrumType(frustrumType);
  int64_t resetDirectionType;
