...
enum class ResetDirectionType: int64_t
{
  plus_X = 0, plus_Y = 1, plus_Z = 2, minus_X = 3, minus_Y = 4, minus_Z = 5
};

enum class FrustrumType: int64_t
{
...
public:
  ...
  double3 myGluUnProject(double3 position, RKRect viewPort);
  double3 myGluUnProject(double3 position, double4x4 modelMatrix, RKRect viewPort);
  double3 myGluProject(double3 position, RKRect viewPort);
  ...
  // The eye, the view matrix and the frustum are derived state and are not archived;
  // a camera read back from a BinaryArchive needs them rebuilt before it can draw.
  ...
  std::vector<size_t> selectPositionsInRectangle(std::vector<double3> &positions, RKRect rect, double3 origin, RKRect viewPortBounds);
private:
  ...
  friend BinaryArchive &operator<<(BinaryArchive &, const std::shared_ptr<RKCamera> &);
  friend BinaryArchive &operator>>(BinaryArchive &, std::shared_ptr<RKCamera> &);
};
