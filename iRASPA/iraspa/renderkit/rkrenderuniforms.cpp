...
// offsets are the ones the Qt version asserts, so both describe the same layout.
static_assert(offsetof(RKStructureUniforms, ribbonCoilColor) == 992, "ribbon block moved");
static_assert(offsetof(RKStructureUniforms, ribbonHDR) == 1040, "ribbon scalars moved");
static_assert(offsetof(RKStructureUniforms, ribbonSaturation) == 1052, "ribbon scalars moved");
static_assert(offsetof(RKStructureUniforms, ribbonAmbientColor) == 1088, "ribbon materials moved");
...
namespace
{
  // Both constructors describe the same structure, so the ribbon state is read in one place.
  void readRibbonState(RKStructureUniforms &uniforms, const std::shared_ptr<RKRenderObject> &structure)
  {
    RKRenderRibbonSource* source = dynamic_cast<RKRenderRibbonSource*>(structure.get());
    if (!source)
    {
      return;
    }

    const float3 coilColor = source->ribbonCoilColor();
    const float3 helixColor = source->ribbonHelixColor();
    const float3 sheetColor = source->ribbonSheetColor();
    uniforms.ribbonCoilColor = float4(coilColor.x, coilColor.y, coilColor.z, 1.0f);
    uniforms.ribbonHelixColor = float4(helixColor.x, helixColor.y, helixColor.z, 1.0f);
    uniforms.ribbonSheetColor = float4(sheetColor.x, sheetColor.y, sheetColor.z, 1.0f);

    uniforms.ribbonHDR = source->ribbonHDR() ? 1 : 0;
    uniforms.ribbonHDRExposure = float(source->ribbonHDRExposure());
    uniforms.ribbonHue = float(source->ribbonHue());
    uniforms.ribbonSaturation = float(source->ribbonSaturation());
    ...
    const RKColor specular = source->ribbonSpecularColor();
    uniforms.ribbonAmbientColor = float(source->ribbonAmbientIntensity()) *
                                  float4(ambient.redF(), ambient.greenF(), ambient.blueF(), ambient.alphaF());
    uniforms.ribbonDiffuseColor = float(source->ribbonDiffuseIntensity()) *
                                  float4(diffuse.redF(), diffuse.greenF(), diffuse.blueF(), diffuse.alphaF());
    uniforms.ribbonSpecularColor = float(source->ribbonSpecularIntensity()) *
                                   float4(specular.redF(), specular.greenF(), specular.blueF(), specular.alphaF());
  }
}
...
RKStructureUniforms::RKStructureUniforms(size_t sceneIdentifier, size_t movieIdentifier, std::shared_ptr<RKRenderObject> structure)
{
  ...
  if (RKRenderObject* renderStructure = dynamic_cast<RKRenderObject*>(structure.get()))
  {
    ...
    readRibbonState(*this, structure);
